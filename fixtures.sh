...
# podman
podman build -t runlike_fixture dockerfiles/

podman network rm runlike_fixture_bridge
podman network create runlike_fixture_bridge

podman rm -f runlike_fixture1
podman run -d --name runlike_fixture1 \
    --hostname Essos \
    --expose 1000 \
    --expose 1000/udp \
    --privileged \
...
podman rm -f runlike_fixture2
podman run -d --name runlike_fixture2 \
    --restart=on-failure \
    --net host \
    --pid host \
    --device=/dev/null:/dev/null \
...
podman rm -f runlike_fixture3
podman run -d --name runlike_fixture3 \
    --restart=on-failure:3 \
    --network runlike_fixture_bridge \
    --log-opt mode=non-blocking \
    --log-opt max-buffer-size=4m \
...
podman rm -f runlike_fixture4
podman run -d --name runlike_fixture4 \
    --restart= \
    --mac-address=6a:00:01:ad:d9:e0 \
    runlike_fixture \
    bash -c "bash 'sleep.sh'"

podman rm -f runlike_fixture5
podman run -d --name runlike_fixture5 \
    --link runlike_fixture4:alias_of4 \
    --link runlike_fixture1 \
    runlike_fixture